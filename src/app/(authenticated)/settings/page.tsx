...
function RulesSection({ isAdmin }: { isAdmin: boolean }) {
  const me = useMe();
...
function TeamSection({ isAdmin }: { isAdmin: boolean }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [profiles, rolesRes] = await Promise.all([
  ...
  const send = useMutation({
    mutationFn: () => inviteTeammateFn({ data: { email: inviteEmail.trim(), roles } }),
        ...
        <div className="rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-xs space-y-5">
          ...
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="inv-email" className="text-xs font-bold text-[#0B1457]">
                  Work Email Address
                </Label>
...
function AuditLogSection() {
  ...
  return (
    <div className="rounded-xl border border-[#E5E7EB] bg-white p-5 shadow-xs">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-bold text-sm text-[#111315]">Audit Trail (SHA-256 Ledger)</h2>
          <p className="text-xs text-[#6B7280]">
            Immutable log of all approvals, threshold evaluations, and configuration changes.
          </p>
        </div>
      ...
      <div className="mt-4 overflow-x-auto">
        {isLoading ? (
          <p className="text-xs text-[#6B7280]">Loading audit records…</p>
        ) : !logs?.length ? (
          <p className="text-xs text-[#6B7280]">No audit entries recorded yet.</p>
        ) : (
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-[#E5E7EB] font-bold text-[#6B7280]">
            ...
            <tbody className="divide-y divide-[#F3F4F6]">
              {logs?.map((l) => (
                <tr key={l.id}>
                  <td className="px-3 py-3 text-[#6B7280]">{dateTime(l.created_at)}</td>
                  <td className="px-3 py-3 font-semibold text-[#111315]">{l.action}</td>
                  <td className="px-3 py-3 text-[#6B7280]">{l.actor_role}</td>
                  <td className="px-3 py-3 font-mono text-[10px] text-[#9CA3AF]">
                    {(l as any).payload_hash || (l as any).entry_hash
                      ? `${((l as any).payload_hash || (l as any).entry_hash).slice(0, 16)}…`
                      : "Verified"}
                  </td>
                </tr>
              ))}
...
function NdpaSection({ isAdmin }: { isAdmin: boolean }) {
  const [retentionYears, setRetentionYears] = useState("7");
  const [loggingConsent, setLoggingConsent] = useState(false);

  async function handleConsent() {
    setLoggingConsent(true);
    try {
      await logNdpaConsentFn({
        data: {
      ...
      toast.success("NDPA consent record logged successfully.");
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Failed logging consent.");
    } finally {
      setLoggingConsent(false);
    }
  }
